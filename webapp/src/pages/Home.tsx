...
function getDashboardRouteForRole(role: Role): string {
...
const Home: React.FC = () => {
  ...
  const getStartedTarget = isAuthenticated ? getDashboardRouteForRole(role) : "/login";
    ...
    <Box sx={{ minHeight: "100vh" }}>
      ...
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "center", px: 2, mt: 5, mb: 6 }}>
        ...
        >
          ...
          >
            ...
            >
              ProceduralNexus helps you upload, manage, and track documents, extract insights with AI, and complete
              secure e-signatures in one place.
            </Typography>

            <Box sx={{ display: "flex", gap: 2, justifyContent: { xs: "center", md: "flex-start" }, width: "100%" }}>
              {/* Get started: dashboard diferit pe rol daca esti logat, altfel register */}
              <Link to={getStartedTarget} style={{ textDecoration: "none" }}>
                <Button
                  variant="contained"
                  sx={{
      ...
      <Container maxWidth="lg" sx={{ mb: 8 }} ref={featuresRef}>
        ...
        >
          ...
          >
            {[
              { icon: Description, title: "Document Management", description: "upload, assign, track" },
              { icon: TrendingUpSharp, title: "AI Analysis", description: "extract insights, automate workflows" },
              { icon: DrawRounded, title: "E-Signature", description: "secure signing, audit trail" },
            ].map((feature, index) => (
              <Box
                key={index}
                sx={{
      ...
      {/* CTA Section - doar cand NU esti conectat */}
      {!isAuthenticated && (
